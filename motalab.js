...
// Garante que a sessão persiste ao recarregar a página (corrige bug mobile)
      ...
      (snap) => {
        ...
        if (d.status === "ativo") {
          ...
          if (telaAtual === "pendingScreen") {
            showToast(
              "🎉 Cartão ativado! Bem-vindo(a), " + primeiroNome(d.nome) + "!",
              "ok",
            );
          } else {
          ...
          signOut(auth).then(() => {
            ...
            showToast(
              "Sua conta está inativa. Entre em contato com o MotaLab.",
              "err",
            );
          });
        }
...
function confirmarAcao(msg, tipo = "danger") {
  return new Promise((resolve) => {
    ...
    if (!subEl) {
      ...
      document.getElementById("confirmMsg").after(subEl);
    }
    subEl.textContent =
      tipo === "logout"
    ...
    // Texto dos botões
    document.getElementById("confirmSim").textContent =
      tipo === "logout" ? "Sair da conta" : "Confirmar";
    ...
    const res = (v) => {
      ...
      sim.removeEventListener("click", hs);
      nao.removeEventListener("click", hn);
      resolve(v);
    };
    const hs = () => res(true);
    const hn = () => res(false);
...
function atualizarNavbar(user) {
  ...
  if (user) {
    ...
    if (badge)
      badge.textContent =
        user.status === "pendente" ? "⏳ Aguardando" : "Cartão Ativo";
...
function abrirRecuperacao() {
  fecharLogin();
  // Limpa campos
  ["recEmail", "recCpf", "recTel"].forEach((id) => {
    const el = document.getElementById(id);
    if (el) {
      el.value = "";
  ...
  // Máscara CPF
  const cpfEl = document.getElementById("recCpf");
  if (cpfEl && !cpfEl._masked) {
    cpfEl._masked = true;
  ...
  // Máscara telefone
  const telEl = document.getElementById("recTel");
  if (telEl && !telEl._masked) {
    telEl._masked = true;
...
function enviarRecuperacao() {
  const eEl = document.getElementById("recEmail");
  const cEl = document.getElementById("recCpf");
  const tEl = document.getElementById("recTel");
  let ok = true;

  const email = eEl.value.trim();
  const cpf = cEl.value.trim();
  const tel = tEl.value.trim();

  /\S+@\S+\.\S+/.test(email)
    ? eEl.classList.remove("err")
    : (eEl.classList.add("err"), (ok = false));
  cpf.replace(/\D/g, "").length === 11
    ? cEl.classList.remove("err")
    : (cEl.classList.add("err"), (ok = false));
  tel.replace(/\D/g, "").length >= 10
    ? tEl.classList.remove("err")
    : (tEl.classList.add("err"), (ok = false));

  if (!ok) {
    showToast("Verifique os campos em vermelho.", "err");
    return;
  }
  ...
  const partes = [
    "Ola, MotaLab!",
    "",
    "Preciso de ajuda para *recuperar a minha senha* de acesso ao Cartao Fidelidade.",
    "",
    ...
    "- Tel: " + tel,
    "",
    "Por favor, me ajudem a redefinir a senha. Obrigado(a)!",
  ];
  const wppUrl =
    "https://wa.me/" +
    CFG.WPP_NUM +
  ...
  fecharRecuperacao();
  showToast("WhatsApp aberto! Envie a mensagem para o MotaLab.", "ok");
}
...
function abrirCadastro() {
  tempCadData = null;
  tempDeps = [];
  ["iNome", "iCpf", "iNasc", "iEmail", "iTel", "iSenha", "iConf"].forEach(
    (id) => {
      const el = document.getElementById(id);
      if (el) {
        el.value = "";
...
function mostrarStep(step) {
  ...
  [1, 2, 3, 4].forEach((n) => {
    ...
    if (el)
      el.className = "step" + (n < cur ? " done" : n === cur ? " active" : "");
  });
...
function goToDeps() {
  ...
  nome.value.trim().split(" ").filter(Boolean).length >= 2
    ? cl(nome)
    : er(nome);
...
function renderizarDepList() {
  ...
  if (cnt)
    cnt.textContent = `${tempDeps.length} de ${CFG.MAX_MBR} vagas preenchidas`;
  if (btn) btn.disabled = tempDeps.length >= CFG.MAX_MBR;
}

function adicionarDepForm() {
  if (tempDeps.length >= CFG.MAX_MBR) {
    showToast(`Limite de ${CFG.MAX_MBR} participantes`, "err");
    return;
  }
...
function confirmarDepForm() {
  ...
  nome.split(" ").filter(Boolean).length >= 2
    ...
    : (nEl.classList.add("err"), (ok = false));
  nasc ? dEl.classList.remove("err") : (dEl.classList.add("err"), (ok = false));
  if (!ok) {
    showToast("Verifique os campos", "err");
    return;
  }
  tempDeps.push({
    nome,
...
async function confirmarPix() {
  ...
  try {
    ...
    const docData = {
      ...
      senhaCadastro: tempCadData.senha, // armazenado para suporte admin
    ...
    unsubUserDoc = onSnapshot(doc(db, "users", createdUser.uid), (snap) => {
      if (!snap.exists()) return;
      const d = snap.data();
    ...
    const msgs = {
      ...
      "auth/operation-not-allowed":
        "⚠️ Ative Email/Password em Authentication → Sign-in method.",
    ...
    if (err.code === "auth/email-already-in-use") {
      ...
      if (["auth/weak-password", "auth/invalid-email"].includes(err.code))
        mostrarStep("cad");
    }
  }
}
...
function preencherHome() {
  ...
  if (u.dataVenc) {
    ...
    document.getElementById("dcVenc").textContent =
      p.length === 3 ? `${p[1]}/${p[2]}` : u.dataVenc;
  }
  ...
  document.getElementById("infoNasc").textContent = u.nasc
    ? formatarDataNasc(u.nasc)
    : "—";
...
function abrirAddMembro() {
  if ((userData?.membros || []).length >= CFG.MAX_MBR) {
    showToast(`Limite de ${CFG.MAX_MBR} participantes`, "err");
    return;
  }
  ["mNome", "mNasc"].forEach((id) => {
    const e = document.getElementById(id);
    if (e) {
      e.value = "";
      e.classList.remove("err");
...
function gerarPayloadPix() {
  ...
  let p =
    ...
    emvF("52", "0000") +
    emvF("53", "986") +
    emvF("54", CFG.VALOR_PIX) +
    emvF("58", "BR") +
    emvF("59", CFG.NOME_PIX.slice(0, 25)) +
    emvF("60", CFG.CIDADE_PIX.slice(0, 15)) +
    emvF("62", emvF("05", "***")) +
    "6304";
...
function copiarPix() {
  ...
  if (navigator.clipboard?.writeText)
    navigator.clipboard
      .writeText(CFG.CHAVE_PIX)
      .then(ok)
      .catch(() => fallbackCopy(ok));
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Fechar modal recuperação clicando fora
  const recOv = document.getElementById("modalRecuperacao");
  if (recOv)
...
document.addEventListener("keydown", (e) => {
  if (
    e.key === "Enter" &&
    document.getElementById("modalLogin")?.classList.contains("open")
  )
    fazerLogin();
});
